...
def save_file():
    with open("student_list.html", "w") as w:
        w.write(str(student_list))
        print("亲，您的数据已成功帮您保存到云端！")


def load_student():
    global student_list  # 提升为全局变量，覆盖全局
    if os.path.exists("student_list.html"):
        with open("student_list.html", "r") as r:
            content = r.read()
            student_list = eval(content)  # eval慎用
        print(student_list)
